...
Route::redirect('/', "/p/{$PROJECT_SLUG}/login");

Route::prefix("p/{$PROJECT_SLUG}")
    ->as($PROJECT_ROUTE_PREFIX)
    ->group(function ()  {
        Route::get('/register', Register::class)->name('auth.register'); // Nombre final: qp.auth.register
        Route::get('/login', Login::class)->name('auth.login');         // Nombre final: qp.auth.login
        Route::get('/forgot-password', ForgotPassword::class)->name('auth.forgot-password'); // Nombre final: qp.auth.forgot-password
